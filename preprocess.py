...
#        print("outliers in",col[i]," after correction  = ",count1)
    df1.to_csv("CSV_Files/outliers_removed.csv",index=None)
    return df1
##################################################
def min_max_normalization(col,df):
    ...
    df2.to_csv("CSV_Files/normalized.csv",index=None)    
#    print(df1)
    return df2
######################################################  
def standardization(col,df):
    ...
    df3.to_csv("CSV_Files/standardised.csv",index=None)
#    print(df2)    
    return df3
######################################################  
def dim_reduction(df,dim):
    ...
    finaldf.to_csv("CSV_Files/dim_reduced_to_"+str(dim)+".csv")
    return finaldf   
